...
export class Token {
    ...
    set(tokenValue: string): any {
        const token = addTokenPrefix(tokenValue, this.strategy.properties.token.type)
    ...
    _updateExpiration(token: string | boolean): any {
        ...
        const _tokenTTLMillis = Number(this.strategy.properties.token.maxAge) * 1000
        ...
        return this._setExpiration(tokenExpiration.toString() || false)
    }
